export const wellnessPageData = {
  heroSection: {
    title: "Holistic Wellness",
    video: "/videos/Holistic-bnr.mp4",
    videoPoster: "/videos/Holistic-bnr.png",
  },
  AboutUsSection: {
    title: "समत्वं योग उच्यते <br/> Evenness of Mind is known as Yoga.",
    cards: [
      {
        description:
          'At Rudraksh Retreat, we offer a holistic wellness experience to nurture every part of your being- mind, body, and spirit. Inspired by the wisdom of "Satyam yoga uchyate," (true balance comes from within), every element of your stay is thoughtfully designed to help you reconnect and rejuvenate in a way that is both authentic and personal.',
        image: "/wellness/Rudraksh-pic-2.png",
      },
      {
        description:
          "Whether you’re indulging in our serene spa treatments, flowing through guided yoga sessions amidst nature, or letting the outdoors soothe you during a peaceful stroll, we’re here to support your journey toward balance, renewal and well-being.",
        image: "/wellness/DJI_20250216.png",
      },
    ],
  },
  cards: [
    {
      image: "/blogs/host-img2.jpg",
      title: "Serene Spa Treatments",
      subtitle: "THE RUDRAKSH WELLNESS PLAN",
      description: [
        "Let your mind and body relax with our calming spa therapies. Each treatment is designed to release tension, rejuvenate your skin, and promote overall well-being. Surrounded by nature, these spa treatments help you reconnect with yourself in a deeply soothing environment, restoring balance to your body.",
      ],
      listTitle: "Massage & Therapy",
      list: [
        "Janu Basti (for Knees) 30-40 Min.",
        "Kati Basti (for Back) 30-40 Min.",
        "Greeva Basti (for Neck) 30-40 Min.",
        "Udar Basti (for Abdomen) 30-40 Min.",
        "Shiro Vasti (for Head) 30-40 Min.",
        "Nasya 20 Minutes",
        "Potli Massage Therapy (Reduces stiffness and soreness that accompanies illnesses like spondylitis, frozen solder rheumatoid arthritis, etc.",
        "Abhyangam Massage 60 Min. (Full Body Massage)",
        "Deep Tissue Massage 60 Min.",
        "Dry Massage 60 Min.",
        "Shirodhara (for Insomnia) 50-60 Min",
      ],
    },
    {
      title: "Sacred Detox & Cleansing Retreat",
      description: [
        "Our yoga sessions, inspired by ancient principles of balance and harmony, offer a deeply enriching experience whether held in our spacious yoga room with serene mountain views or outdoors amidst nature.",
        "The Rudraksh Yogshala is equipped with essentials, including yoga mats, blocks, bolsters, Iyengar chairs, and blankets. For a unique therapeutic touch, we also provide Tibetan bowls, tongue drums, and tabla sets for sound healing therapy.",
        "With its perfect blend of space and ambiance, this is an ideal setting for yogis to host transformative yoga retreats.",
      ],
    },
    {
      title: "THE RUDRAKSH YOGA EXPERIENCES",
      description: [
        "Harmonize your mind, body, and spirit through the timeless wisdom of yoga and Ayurveda with our rejuvenating Yogic Detox. This retreat enhances self-awareness, inner harmony, and mental clarity, while boosting physical energy and managing stress. You'll leave feeling deeply connected to nature, with a radiant inner and outer glow, and an overall sense of well-being.",
      ],
      image: "/wellness/service-flx-rgt.jpg",
    },
    {
      image: "/blogs/DSC07562-scaled.jpg",
      title: "Ayurveda Panchakarma (7 or 10 Nights)",
      subtitle: "Benefits:",
      description: [
        "A sacred cleansing ritual that purifies the body and restores the spirit, Panchakarma follows a specific protocol with a defined minimum duration. This program offers a transformative escape, blending the wisdom of Ayurveda with modern wellness practices for deep detoxification and rejuvenation.",
        "A sacred cleansing ritual that purifies the body and restores the spirit, Panchakarma follows a specific protocol with a defined minimum duration. This program offers a transformative escape, blending the wisdom of Ayurveda with modern wellness practices for deep detoxification and rejuvenation.",
      ],
      description2: [
        "This retreat helps you address emotional, physical, and energetic imbalances, guiding you in restoring harmony with your natural energy. It targets the root of life’s challenges and supports the development of healthy habits for long-term well-being and longevity.",
      ],
      listTitle: "INCLUSIONS",
      list: [
        "Traditional Ayurvedic cleansing treatments, curated to detoxify",
        "Yoga, Pranayama, and Meditation to renew body and mind",
        "Pre-stay diagnostics and tailored wellness assessments",
        "Personalized Ayurveda and Physiotherapy sessions",
        "Continuous health assessments to monitor progress",
        "Pre-departure wellness briefing for sustained benefits",
        "Follow-up consultations to ensure lasting rejuvenation",
      ],
    },
  ],
  packages: [
    {
      link: "/Downloadable-Subpage-Holistic-Wellness.pdf",
      description: "3 Nights Rudraksh Yogic Detox",
    },
    {
      link: "/Downloadable-Subpage-Holistic-Wellness.pdf",
      description: "5 Nights Rudraksh Yogic Detox",
    },
    {
      link: "/Downloadable-Subpage-Holistic-Wellness.pdf",
      description: "7 Nights Rudraksh Yogic Detox",
    },
  ],
  natureWalks: {
    title: "Nature Walks & Himalayan Trails!",
    image: "/wellness/sami.jpg",
    description: [
      "Let the Himalayan landscape soothe your mind with peaceful nature walks. Just 15 minutes from The Rudraksh Retreat, discover the enchanting lord Shiva’s temple ‘Agaan’, set in a meadow with breathtaking sunrise and sunset.",
      "Explore more nature trails, including the Kaudia Forest and nearby village paths, each offering a unique, rejuvenating experience. Casper and Misty, our beloved Golden Retrievers, may also joyfully accompany you on your walk.",
    ],
  },
};
