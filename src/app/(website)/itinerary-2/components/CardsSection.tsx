  ...
  return (
    <div className="lg:w-1/2 flex flex-col lg:items-center items-end">
      <div className="relative space-y-4 lg:w-[80%] w-[75%] bg-white border-x-[0.2px] border-y-3 shadow-lg border-primary rounded-2xl md:p-8 p-4">
        <p className={`text-lg text-p22 max-md:text-center ${day % 2 !== 0 ? "lg:text-right" : ""}`}>{subtitle}</p>
        <p className={`text-[#626262] max-md:text-center uppercase  ${day % 2 !== 0 ? "lg:text-right" : ""}`}>day {day}</p>
        <p className={`lg:text-3xl text-2xl text-secondary max-md:text-center ${day % 2 !== 0 ? "lg:text-right" : ""}`}>
          {title}
        </p>
        <p className={`text-lg max-md:text-center text-secondary ${day % 2 !== 0 ? "lg:text-right" : ""}`}>{description}</p>
        <div
